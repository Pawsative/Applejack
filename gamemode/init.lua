...
include("external/sv_mysqlite.lua")
include("sv_mysql.lua") -- SQL Shit
...
do
    ...
    function hook.Call(name, gm, ply, text, ...)
...
hook.Add("DatabaseInitialized", "inventory", function()
    GAMEMODE:Query([[CREATE TABLE IF NOT EXISTS cider_inventory(
    ...
    GAMEMODE:Query([[CREATE TABLE IF NOT EXISTS `players` (
...
function GM:Query(query, callback)
    if query then
        MySQLite.query(query, function(data, lastVal)
            if callback then
                callback(data)
            end
        end, function(err)
...
function GM:PhysgunPickup(ply, entity)
    if (entity:IsValid() and entity.PhysgunPickup) then
    ...
    elseif entity.PhysgunDisabled then
        return false
    elseif self.entities[entity] and not ((entity:GetClass() == "prop_physics" or entity:GetClass() == "prop_physics_multiplayer") and ply:IsAdmin()) then
        return false
    ...
    if (string.find(entity:GetClass(), "npc_") or string.find(entity:GetClass(), "cider_") or string.find(entity:GetClass(), "prop_dynamic")) then return false end
    -- Call the base class function.
...
function GM:PlayerSpawnProp(ply, model)
    ...
    if (radius > 100 and not ply:HasAccess("e")) or (radius > 200 and not ply:HasAccess("m")) or (radius > 300) then
        --Only mods go above 200
        --Only admins go above 300.
        ply:Notify("That prop is too big!", 1)

        return false
    end

    --Only donators go above 100
...
function GM:PlayerCanDoSomething(ply, ignorealive, spawning)
    ...
    if (not ply:Alive() and not ignorealive) or ply:Arrested() or ply:KnockedOut() or ply:Tied() or ply._Stunned or ply._HoldingEnt or ply._Equipping or ply._StuckInWorld or spawning and (ply:InVehicle()) then
        --[[or other places they shouldn't spawn shit]]
...
function GM:IsListenServer()
    if (isListen ~= nil) then return isListen end
...
function GM:CanTool(ply, trace, tool, nailee)
    ...
    if (IsValid(trace.Entity)) then
        ...
        if tool ~= "remover" and not ply:IsAdmin() then
            ...
            if string.find(class, "camera") or cider.entity.isDoor(trace.Entity, true) or string.find(class, "vehicle") then return false end
        end
        ...
        if (trace.Entity.CanTool) then return trace.Entity:CanTool(ply, trace, tool) end

        if tool == "remover" and trace.Entity._Removeable and cider.entity.isDoor(trace.Entity) and cider.entity.isOwned(trace.Entity) and type(cider.entity.getOwner(trace.Entity)) == "Player" and not ply:KeyDown(IN_RELOAD) then
        ...
        if not ply:HasAccess("w") and string.sub(tool, 1, 5) == "wire_" then
        ...
        if (tool == "nail" and not nailee) then
            ...
            if (IsValid(line.Entity)) then
                if self.entities[line.Entity] or not hook.Call("CanTool", GAMEMODE, ply, line, tool, true) then return false end
            end
        end
...
function GM:EntityTakeDamage(entity, damageInfo)
    ...
    if (attacker:IsPlayer() and (attacker:GetMoveType() == MOVETYPE_NOCLIP or attacker._StuckInWorld)) or (entity:IsPlayer() and entity:GetMoveType() == MOVETYPE_NOCLIP and not entity:InVehicle()) or (entity:IsPlayer() and entity._Physgunnin) then
        damageInfo:SetDamage(0)
    ...
    if (entity:IsPlayer()) then
        if (entity:KnockedOut()) then
            ...
            --Thanks gayry for breaking teh pains in vehicles.
            -- stop the engine doing anything odd
            ...
            if entity._LastHitGroup and (not attacker:IsPlayer() or (IsValid(attacker:GetActiveWeapon()) and attacker:GetActiveWeapon():GetClass() ~= "cider_hands")) then
                ...
                elseif (entity._LastHitGroup == HITGROUP_LEFTARM or entity._LastHitGroup == HITGROUP_RIGHTARM or entity._LastHitGroup == HITGROUP_LEFTLEG or entity._LastHitGroup == HITGROUP_RIGHTLEG or entity._LastHitGroup == HITGROUP_GEAR) then
    ...
    elseif (entity:IsNPC()) then
        if (attacker:IsPlayer() and IsValid(attacker:GetActiveWeapon()) and attacker:GetActiveWeapon():GetClass() == "weapon_crowbar") then
    ...
    elseif (IsValid(entity._Player) and not entity._Corpse) then
        ...
        if (entity._Player._ScaleDamage and attacker ~= game.GetWorld()) then
...
function GM:ShowTeam(ply)
    ...
    if not (IsValid(door) and cider.entity.isOwnable(door) and ply:GetPos():Distance(ply:GetEyeTraceNoCursor().HitPos) <= 128) then return end
